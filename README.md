...
Edit **`pbs-backup.toml`**, then run **`sudo ./pbs-backup`**. Settings flags are not needed. The app backs up VMs and containers through `vzdump`, keeps full logs in a **`logs/` folder beside the script**, writes **only error messages to `.err`**, and attempts to publish the backup result to MQTT.
...
Open `pbs-backup.toml` in your editor. Fill these existing values; do not append duplicate tables or keys:
...
- The default config is `pbs-backup.toml` beside the **real script file**, even when invoked from another working directory or through a symbolic link.
...
- TOML values are not shell commands, and environment variables such as `$PASSWORD` are not expanded. Passwords stored in TOML stay off the process command line, but the file is plaintext. Use `chmod 600 pbs-backup.toml` to restrict access before saving secrets.
...
All 30 settings are present and commented in `pbs-backup.toml`. The tables show their meanings and optional CLI equivalents. Boolean CLI switches enable their setting; to disable a TOML-enabled boolean, edit TOML. `--no-all` is the explicit selection exception.
...
Both options default to `false` and work independently. To test both, edit the existing sections in `pbs-backup.toml`:
...
| `chmod 600 pbs-backup.toml` | Restrict read/write access to the file owner before storing credentials. |
